-- Fix the cards table to ensure all required columns exist for the scraper
-- This script adds any missing columns that the scraper expects
...
-- Update existing cards to use card_set as set_name if set_name is null
UPDATE cards 
SET set_name = card_set 
WHERE set_name IS NULL AND card_set IS NOT NULL;

-- Update existing cards to have proper grading if it's null
UPDATE cards 
SET grading = condition 
WHERE grading IS NULL AND condition IS NOT NULL;
